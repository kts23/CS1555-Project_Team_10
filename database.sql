...
date_of_birth   date,
lastlogin       timestamp,
CONSTRAINT PROFILE_CH CHECK (userID>0),
...
CONSTRAINT FRIENDS_CH CHECK (userID1<>userID2),
...
message     varchar2(200),
CONSTRAINT PENDINGFRIENDS_CH CHECK (fromID<>toID),
...
CONSTRAINT MESSAGES_CH CHECK (fromID><toUserID),
...
CREATE OR REPLACE TRIGGER firends_constraint_trigger
AFTER INSERT, UPDATE of toID1
ON PENDINGFRIENDS
FOR EACH ROW
WHEN (EXISTS(SELECT f.userID2 
	     FROM FRIENDS f,PENDINGFRIENDS p
	     WHERE f.userID1=fromID))
ROLLBACK;
/

---CREATING PENDINGGROUPMEMBERS TRIGGER
CREATE OR REPLACE TRIGGER memberships_constraint_trigger
AFTER INSERT, UPDATE of userID
ON PENDINGGROUPMEMBERS
FOR EACH ROW
WHEN (EXISTS(SELECT g.userID 
	     FROM GROUPMEMBERSHIP g,PENDINGGROUPMEMBERS p
	     WHERE g.gID=p.gID))
ROLLBACK;
/
